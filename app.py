...
Rotondi Group Roma
Flask + SQLite + Google Maps + Telegram
"""
...
def calcola_preventivo(indirizzo_cliente):
    try:
        ...
        return {
            ...
            "testo": (f"Fuori Roma — {el['distance']['text']} ({el['duration']['text']}) "
                      f"— min. EUR {costo:.2f} + IVA"),
            "dist_label": el["distance"]["text"],
            "dur_label":  el["duration"]["text"],
            "dettaglio": {
                "km_ar":          f"{dist_ar:.0f}",
...
<div style="background:#fff;padding:32px;border-radius:0 0 8px 8px">
  ...
  <div style="background:#fff3cd;border-radius:8px;padding:12px;margin:16px 0">
    ...
    📞 <b>+39 06 41 40 0514</b></p>
  </div>
  <p style="color:#666;font-size:13px;margin-top:24px">
    Ufficio Roma: +39 06 41400617<br>
    <a href="mailto:assistenza@rotondigroup.it">assistenza@rotondigroup.it</a>
  </p>
...
@app.route("/")
def index():
    condizioni_it = get_config("condizioni_it", CONDIZIONI_IT_DEFAULT)
    condizioni_en = get_config("condizioni_en", CONDIZIONI_EN_DEFAULT)
    return render_template_string(HTML_FORM,
                                  condizioni_it=condizioni_it,
                                  condizioni_en=condizioni_en)
...
def route_invia():
    ...
    # Preventivo per messaggio Telegram
    prev_text = ""
    if prev_json:
        try:
    ...
    return jsonify({"protocollo": protocollo})
...
.lang-bar a{margin:0 8px;font-size:13px;color:#666;text-decoration:none;cursor:pointer;padding:4px 8px;border-radius:4px}
.lang-bar a.active{color:#fff;background:#0d0d14;font-weight:700}
...
input,select,textarea{width:100%;padding:10px 12px;border:1.5px solid #ddd;border-radius:8px;font-size:14px;outline:none;transition:border .2s;font-family:inherit}
...
.btn{width:100%;background:#0d0d14;color:#fff;border:none;padding:14px;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;transition:opacity .2s;margin-top:4px}
...
.prev-box{border-radius:10px;padding:16px;margin:14px 0;display:none;border:1.5px solid}
...
.cond-box{background:#f8f8f8;border:1px solid #ddd;border-radius:8px;padding:14px;font-size:13px;line-height:1.7;max-height:180px;overflow-y:auto;white-space:pre-wrap;margin-bottom:12px}
.loading{display:none;text-align:center;padding:10px;font-size:13px;color:#666}
.spin{display:inline-block;width:16px;height:16px;border:2px solid #ddd;border-top-color:#0d0d14;border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle;margin-right:6px}
...
.ok-proto{font-size:26px;font-weight:700;color:#0d0d14;background:#f0f0f0;padding:10px 24px;border-radius:8px;display:inline-block;margin:14px 0;letter-spacing:2px}
...
<div class="lang-bar">
  <a onclick="setLang('it')" id="l_it" class="active">🇮🇹 IT</a>
  <a onclick="setLang('en')" id="l_en">🇬🇧 EN</a>
  <a onclick="setLang('bn')" id="l_bn">🇧🇩 বাং</a>
  <a onclick="setLang('zh')" id="l_zh">🇨🇳 中</a>
  <a onclick="setLang('ar')" id="l_ar">🇸🇦 ع</a>
</div>
<div class="container">
  <div class="steps">
...
Conservazione: max 2 anni. Diritti: accesso, rettifica, cancellazione.</div>
  ...
  <div id="step2" style="display:none">
    ...
    <div class="card">
      ...
      <button class="btn" style="background:#555;margin-bottom:12px" onclick="calcolaPreventivo()" id="btn_calc">
        📍 Verifica distanza e preventivo
      </button>
      <div class="loading" id="loading_p"><span class="spin"></span><span id="t_calc_lbl">Calcolo in corso...</span></div>
...
var COND={'it':{{ condizioni_it|tojson }},'en':{{ condizioni_en|tojson }}};
var L={
  it:{gdpr_h:'🔒 Privacy (GDPR)',gdpr_lbl:'Accetto il trattamento dei dati personali ai sensi del GDPR',
    ...
    ok_h:'Richiesta Inviata!',ok_p:'Un tecnico Rotondi Group Roma ti contatterà a breve.<br><br>Per annullare urgentemente:<br><strong>📞 +39 06 41 40 0514</strong>',
  ...
  en:{gdpr_h:'🔒 Privacy (GDPR)',gdpr_lbl:'I accept the processing of personal data under GDPR',
    ...
    ok_h:'Request Sent!',ok_p:'A Rotondi Group Roma technician will contact you shortly.<br><br>To cancel urgently:<br><strong>📞 +39 06 41 40 0514</strong>',
    err_consent:'You must accept privacy policy and conditions to continue',
    err_campi:'Please fill all required fields (*)'},
  bn:{gdpr_h:'🔒 গোপনীয়তা',gdpr_lbl:'আমি GDPR অনুযায়ী সম্মতি দিচ্ছি',
    ...
    ok_h:'অনুরোধ পাঠানো হয়েছে!',ok_p:'টেকনিশিয়ান শীঘ্রই যোগাযোগ করবেন।<br><br>বাতিল: <strong>📞 +39 06 41 40 0514</strong>',
  ...
  ar:{gdpr_h:'🔒 الخصوصية',gdpr_lbl:'أوافق على معالجة البيانات وفق GDPR',
...
function setLang(l){
  lang=l;
  document.querySelectorAll('.lang-bar a').forEach(a=>a.classList.remove('active'));
  document.getElementById('l_'+l).classList.add('active');
  var t=L[l];
  var m={'t_gdpr_h':'gdpr_h','t_gdpr_lbl':'gdpr_lbl','t_cond_h':'cond_h','t_cond_lbl':'cond_lbl',
  ...
  if(COND[l]) document.getElementById('cond_box').textContent=COND[l];
}
...
function goStep3(){
  ...
  for(var i=0;i<f.length;i++){
    if(!document.getElementById(f[i]).value.trim()){
...
function calcolaPreventivo(){
  ...
  fetch('/calcola-preventivo',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({indirizzo:ind})})
  ...
  }).catch(()=>{document.getElementById('loading_p').style.display='none';});
}
...
function invia(){
  ...
  fetch('/invia',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)})
  .then(r=>r.json()).then(data=>{
    if(data.protocollo){
      document.getElementById('step3').style.display='none';
  ...
  }).catch(()=>{btn.disabled=false;btn.textContent=L[lang].btn3;});
}
</script>
</body>
</html>"""
  ...
  <div class="card" style="margin-top:24px">
    ...
    <table>
      ...
      <tr>
        ...
        <td>{{ r[3] }}</td>
        <td>{{ r[4] }}</td>
        <td style="max-width:160px;font-size:12px">{{ r[5][:60] }}{% if r[5]|length > 60 %}…{% endif %}</td>
        <td>
